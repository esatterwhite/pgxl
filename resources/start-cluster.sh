...
/usr/local/pgsql/bin/postgres --datanode -p 15434 -c pooler_port=40103 -D /usr/local/pgsql/data_datanode_3 >logfile 2>&1 &
...
/usr/local/pgsql/bin/psql -c "CREATE NODE datanode_3 WITH (TYPE = 'datanode', PORT = 15434, HOST='0.0.0.0')" postgres
...
/usr/local/pgsql/bin/psql -c "EXECUTE DIRECT ON (datanode_1) 'CREATE NODE datanode_3 WITH (TYPE = ''datanode'', PORT = 15434)'" postgres
...
/usr/local/pgsql/bin/psql -c "EXECUTE DIRECT ON (datanode_2) 'CREATE NODE datanode_3 WITH (TYPE = ''datanode'', PORT = 15434)'" postgres
/usr/local/pgsql/bin/psql -c "EXECUTE DIRECT ON (datanode_2) 'CREATE NODE coord1 WITH (TYPE = ''coordinator'', PORT = 5432)'" postgres

echo "setup node_3"
/usr/local/pgsql/bin/psql -c "EXECUTE DIRECT ON (datanode_3) 'ALTER NODE datanode_3 WITH (TYPE = ''datanode'', PORT = 15434)'" postgres
/usr/local/pgsql/bin/psql -c "EXECUTE DIRECT ON (datanode_3) 'CREATE NODE datanode_1 WITH (TYPE = ''datanode'', PORT = 15432)'" postgres
/usr/local/pgsql/bin/psql -c "EXECUTE DIRECT ON (datanode_3) 'CREATE NODE datanode_2 WITH (TYPE = ''datanode'', PORT = 15433)'" postgres
/usr/local/pgsql/bin/psql -c "EXECUTE DIRECT ON (datanode_3) 'CREATE NODE coord1 WITH (TYPE = ''coordinator'', PORT = 5432)'" postgres
...
/usr/local/pgsql/bin/psql -c "EXECUTE DIRECT ON (datanode_3) 'SELECT pgxc_pool_reload()'" postgres
echo "setup done!"
